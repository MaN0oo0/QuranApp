...
;
    ...
    flex-direction: row-reverse;"><div class="text-start "><a onclick="loadSurah(${server_data.data.number > 0 ? server_data.data.number - 1 : 1})"   class="btn btn-primary ${server_data.data.number - 1 == 0 ? "disabled" : ""}">رجوع</a></div><div class="text-end "><a onclick="loadSurah(${server_data.data.number < 114 ? server_data.data.number + 1 : 114})"   class="btn btn-primary ${server_data.data.number == 114 ? "disabled" : ""}">التالي</a></div></div> <center><h3 id="renderSurah_${server_data.data.number}">${server_data.data.name}</h3></center>`;
  cartoona += btn;
  var ayah = "";
